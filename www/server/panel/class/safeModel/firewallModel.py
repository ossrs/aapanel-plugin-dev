...
class main(safeBase):
    ...
    def get_firewall_status(self, get):
        if self.__isUfw:
            res = public.ExecShell('{} status verbose'.format(self.__ufw))[0]
            ...
            if ports.find(',') != -1:
                    ...
                    if end_port:
                        if int(port) >= int(start_port) and int(port) <= int(
                            ...
                            return public.returnMsg(
                                False, '{} port is already in port {}, please do not add it repeatedly！'.format(
                                    port, i2['ports']))
                    else:
                        if port == start_port:
                            return public.returnMsg(
                                False, 'The {} port has already been added, please do not add it again!'.format(port))
            elif ports.find('-') != -1:
                ...
                if end_port:
                    if (int(add_start) >= int(start_port)
                        ...
                        return public.returnMsg(
                            False, 'The {} port range overlaps with the existing firewall rule port {}, please do not add it repeatedly!'.format(
                                ports, i2['ports']))
                else:
                    if add_start <= start_port and add_end >= start_port:
                        return public.returnMsg(
                            False, 'The {} port range includes the added port {}, please do not add it repeatedly!'.format(
                                ports, i2['ports']))
            else:
                if end_port:
                    if int(ports) >= int(start_port) and int(ports) <= int(
                        ...
                        return public.returnMsg(
                            False,
                            '{} port is already in port {}, please do not add it again!'.format(ports, i2['ports']))
                else:
                    if ports == start_port:
                        return public.returnMsg(
                            False, 'The {} port has already been added, please do not add it again!'.format(ports))
        return {}
    ...
    def add_ufw_rule(self, address, protocol, ports, types):
        ...
        if address == "":
            if protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' ' + rule + ' ' + ports + '/tcp')
                public.ExecShell(self.__ufw+' ' + rule + ' ' + ports + '/udp')
            else:
                public.ExecShell(self.__ufw+' ' + rule + ' ' + ports + '/' + protocol +
                                 '')
        else:
            if protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' ' + rule + ' proto tcp from ' + address +
                                 ' to any port ' + ports + '')
                public.ExecShell(self.__ufw+' ' + rule + ' proto udp from ' + address +
                                 ' to any port ' + ports + '')
            else:
                public.ExecShell(self.__ufw+' ' + rule + ' proto ' + protocol +
    ...
    def del_ufw_rule(self, address, protocol, ports, types):
        ...
        if address == "":
            if protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' delete ' + rule + ' ' + ports + '/tcp')
                public.ExecShell(self.__ufw+' delete ' + rule + ' ' + ports + '/udp')
            else:
                public.ExecShell(self.__ufw+' delete ' + rule + ' ' + ports + '/' +
                                 protocol + '')
        else:
            if protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' delete ' + rule + ' proto tcp from ' +
                                 address + ' to any port ' + ports + '')
                public.ExecShell(self.__ufw+' delete ' + rule + ' proto udp from ' +
                                 address + ' to any port ' + ports + '')
            else:
                public.ExecShell(self.__ufw+' delete ' + rule + ' proto ' + protocol +
                                 ' from ' + address + ' to any port ' + ports +
                                 '')
        self.update_panel_data(ports)
    ...
    def edit_ufw_rule(self, _address, _protocol, _port, _type, address,
        ...
        if _address == "":
            if _protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' delete ' + _rule + ' ' + _port + '/tcp')
                public.ExecShell(self.__ufw+' delete ' + _rule + ' ' + _port + '/udp')
            else:
                public.ExecShell(self.__ufw+' delete ' + _rule + ' ' + _port + '/' +
                                 _protocol + '')
        else:
            if _protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' delete ' + _rule + ' proto tcp from ' +
                                 _address + ' to any port ' + _port + '')
                public.ExecShell(self.__ufw+' delete ' + _rule + ' proto udp from ' +
                                 _address + ' to any port ' + _port + '')
            else:
                public.ExecShell(self.__ufw+' delete ' + _rule + ' proto ' +
                                 _protocol + ' from ' + _address +
                                 ' to any port ' + _port + '')
        if address == "":
            if protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' ' + rules + ' ' + ports + '/tcp')
                public.ExecShell(self.__ufw+' ' + rules + ' ' + ports + '/udp')
            else:
                public.ExecShell(self.__ufw+' ' + rules + ' ' + ports + '/' +
                                 protocol + '')
        else:
            if protocol.find('/') != -1:
                public.ExecShell(self.__ufw+' ' + rules + ' proto tcp from ' +
                                 address + ' to any port ' + ports + '')
                public.ExecShell(self.__ufw+' ' + rules + ' proto udp from ' +
                                 address + ' to any port ' + ports + '')
            else:
                public.ExecShell(self.__ufw+' ' + rules + ' proto ' + protocol +
                                 ' from ' + address + ' to any port ' + ports +
                                 '')
            ...
            if self.__isUfw:
                ...
                if address.find('-') != -1:
                    ...
                    if not is_debian:
                        if _rule == "allow":
                            if public.is_ipv6(address):
                                public.ExecShell(self.__ufw+' ' + _rule + ' from ' +
                                                 address + ' to any')
                            else:
                                public.ExecShell(self.__ufw+' insert 1 ' + _rule +
                                                 ' from ' + address +
                                                 ' to any')
                        else:
                            public.ExecShell(self.__ufw+' ' + _rule + ' from ' +
            ...
            if self.__isUfw:
                ...
                if address.find('-') != -1:
                    ...
                    is_debian = True if public.get_os_version().lower().find(
                        "debian") != -1 else False
                    if not is_debian:
                        public.ExecShell(self.__ufw+' delete ' + _rule + ' from ' +
                                         address + ' to any')
                    else:
                        public.ExecShell("iptables -D INPUT -s " + address +
                                         " -j " + types.upper())
        ...
        if self.__isUfw:
            ...
            if _address.find('-') != -1:
                ...
                is_debian = True if public.get_os_version().lower().find(
                    "debian") != -1 else False
                if not is_debian:
                    public.ExecShell(self.__ufw+' delete ' + rule1 + ' from ' +
                                     address + ' to any')
                else:
            ...
            if address.find('-') != -1:
                ...
                if not is_debian:
                    if rule2 == "allow":
                        if public.is_ipv6(address):
                            public.ExecShell(self.__ufw+' ' + rule2 + ' from ' +
                                             address + ' to any')
                        else:
                            public.ExecShell(self.__ufw+' insert 1 ' + rule2 +
                                             ' from ' + address + ' to any')
                    else:
                        public.ExecShell(self.__ufw+' ' + rule2 + ' from ' + address +
                                         ' to any')
                else:
                    public.ExecShell('iptables -I INPUT -s ' + address +
                                     ' -j ' + types.upper())
        ...
        if isinstance(data_list, str):
            try:
                ...
                if os.path.exists(file_path):
                    os.remove(file_path)
                return public.ReturnMsg(False, "The content of the file is wrong!")
    ...
    def delete_service(self):
        if self.__isUfw:
            public.ExecShell(self.__ufw+' delete allow ssh')
        else:
            if self.__isFirewalld:
                public.ExecShell(
