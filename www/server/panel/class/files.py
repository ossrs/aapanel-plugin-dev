...
session.save_handler = files'''.format(path, sess_path, sess_path)
    ...
    def upload(self, args):
        ...
        try:
            if self.get_real_len(args.f_name) > 128: return public.return_msg_gettext(False,
                                                                                      'The file name contains more than 128 bytes')
        except:
            pass
        if not self.f_name_check(args.f_name): return public.return_msg_gettext(False,
                                                                                'No special characters can be included in the file name!')
    ...
    def CreateLink(self, get):
        ...
        if os.path.exists(get.dfile): return public.return_msg_gettext(False,
                                                                       'The specified soft link file name already exists')
        l_name = os.path.basename(get.dfile)
        if re.match(r"^[\w\-\.]+$", l_name) == None: return public.returnMsg(False, 'Link file name is illegal!')
        if get.dfile[0] != '/': return public.return_msg_gettext(False,
                                                                 'The specified soft link file name must contain the full path (full path)')
    ...
    def GetFileBody(self, get):
        ...
        if data["size"] > 3145928:
            try:
                ...
                if info_data == "": return public.return_msg_gettext(False,
    ...
    def SaveFileBody(self, get):
        ...
        if 'st_mtime' in get:
            ...
            if st_mtime != get['st_mtime']: return public.returnMsg(False,
        ...
        if get.type == '1':
            ...
            for key in myfiles:
                ...
                if dfile.find(sfile) == 0:
                    return public.return_msg_gettext(False,
                                                     'Wrong copy logic, from {} copy to {} has an inclusive relationship, there is an infinite loop copy risk!'.format(
                                                         sfile, dfile))
   ...
   first_path CHAR
    ...
    def back_path_permissions(self, get):
        ...
        if self._get_total_back() >= back_limit:
            return public.return_msg_gettext(False,
                                             "The number of backup versions has exceeded {} ,Please go to the upper right corner [Backup Permissions] to clean up the old backup before operating".format(
                                                 back_limit))
        if not os.path.exists(get.path):
