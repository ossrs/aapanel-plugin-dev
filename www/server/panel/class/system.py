...
class system:
    ...
    def GetPanelInfo(self, get=None):
        ...
        return {'port': port, 'address': address, 'domain': domain, 'auto': autoUpdate, '502': check502,
                'limitip': limitip, 'templates': templates, 'template': template, 'admin_path': admin_path}
    ...
    def GetNetWork(self, get=None):
        ...
        if get != False:
            ...
            for disk in self.GetDiskInfo2(False):
                ...
                disk['size'] = list(
                    map(lambda num: f"{round(int(num) / 1048576, 2)}G" if str(num).isdigit() and str(num).find(
                        'G') == -1 else num, disk['size']))
                disk['inodes'] = list(
                    map(lambda num: f"{round(int(num) / 1048576, 2)}G" if str(num).isdigit() and str(num).find(
                        'G') == -1 else num, disk['inodes']))
    ...
    def GetNetWorkOld(self):
        ...
        networkInfo = {}
        networkInfo['upTotal'] = networkInfo['downTotal'] = networkInfo['up'] = networkInfo['down'] = networkInfo[
    ...
    def ServiceAdmin(self, get=None):
        ...
        if get.name == 'apache' or get.name == 'httpd':
            get.name = 'httpd'
            if not os.path.exists(self.setupPath + '/apache/bin/apachectl'): return public.return_msg_gettext(True,
                                                                                                              'Execution failed, check if Apache installed')
        ...
        # 检查nginx配置文件
            ...
            result = public.ExecShell(
                'ulimit -n 8192 ; ' + self.setupPath + '/nginx/sbin/nginx -t -c ' + self.setupPath + '/nginx/conf/nginx.conf')
    ...
    def RestartServer(self, get):
        if not public.IsRestart(): return public.return_msg_gettext(False,
    ...
    def RepPanel(self, get):
        ...
        public.ExecShell(
            "wget --no-check-certificate -O update.sh " + public.get_url() + "/install/update6_en.sh && bash update.sh")
        self.ReWeb(None)
        return True
